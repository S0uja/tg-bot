...
def register(router: Router, ctx: TelegramContext) -> None:
    ...
    async def test_prompt_builder(message: types.Message, state: FSMContext):
        """Test prompt building against the final pose-library JSON."""
        raw = (message.text or "").strip()
        match = re.match(
        ...
        if library_path.is_file():
            ...
            # Backward-compatible fallback for installations that still only have
            # .pose_library.json generated by /analyze_poses.
            for depth_path in sorted(root_dir.rglob("*_depth.png")):
                metadata = image_service.pose_library.get_analysis(depth_path)
                if metadata:
                    entries.append({
        ...
        if not entries:
            await message.answer(
                "❌ Не найден pose_library_final_v6.json и нет кешированных анализов поз."
            )
            return
        ...
        candidates_paths: list[tuple[Path, dict]] = []
        for path in depth_files:
            metadata = by_filename.get(path.name)
            if metadata:
                candidates_paths.append((path, metadata))

        if not candidates_paths:
            await message.answer(
                "❌ Ни одна *_depth.png карта не сопоставилась с pose_library_final_v6.json."
            )
            return

        enhancer = image_service.prompt_service.enhancer

        # First reduce 505 entries to the requested coarse posture. The final
        # selection uses the new pose_style + support/arms/legs/orientation/
        # framing/activity_tags/keywords_ru fields.
        requested_posture = await enhancer.classify_scene_posture(scene)
        compatible = [
            item for item in candidates_paths
            if requested_posture == "unknown"
            or str(item[1].get("posture", "unknown")).strip().lower() == requested_posture
        ]
        if not compatible:
            compatible = candidates_paths

        scene_words = {
            word for word in re.findall(r"[a-zа-яё]{3,}", scene.lower())
            if word not in {
                "девушка", "женщина", "девушку", "женщину", "картинка",
                "изображение", "фото", "фотография",
            }
        }

        def searchable(metadata: dict) -> str:
            values = [
                metadata.get("posture", ""),
                metadata.get("pose_style", ""),
                metadata.get("orientation", ""),
                metadata.get("support", []),
                metadata.get("arms", ""),
                metadata.get("legs", ""),
                metadata.get("framing", ""),
                metadata.get("activity_tags", []),
                metadata.get("keywords_ru", []),
            ]
            parts: list[str] = []
            for value in values:
        ...
        def candidate_score(item: tuple[Path, dict]) -> int:
            _, metadata = item
            text = searchable(metadata)
            return sum(1 for word in scene_words if word in text)

        ranked = sorted(compatible, key=candidate_score, reverse=True)

        # Keep the second Qwen request small enough for the 8192 context while
        # still exposing the new v6 semantic fields to the model.
        shortlist = ranked[:16]
        candidates = []
        for index, (candidate, metadata) in enumerate(shortlist, start=1):
            candidates.append({
                "index": index,
                "filename": candidate.name,
                "posture": metadata.get("posture", "unknown"),
                "pose_style": metadata.get("pose_style", "unknown"),
                "orientation": metadata.get("orientation", "unknown"),
                "support": metadata.get("support", [])[:4],
                "arms": str(metadata.get("arms", "unknown"))[:140],
                "legs": str(metadata.get("legs", "unknown"))[:140],
                "framing": metadata.get("framing", "unknown"),
                "activity_tags": metadata.get("activity_tags", [])[:6],
                "keywords_ru": metadata.get("keywords_ru", [])[:8],
        ...
        pose_orientation = str(pose_metadata.get("orientation", "")).strip()

        prompt = await image_service.prompt_service.build_image_prompt(
            ImagePromptContext(
                character_description=character.description,
                scene=scene,
                pose=str(pose_metadata.get("pose_style", "")).strip(),
                clothing="",
        ...
        await message.answer(
            f"<b>🧪 Prompt Builder v6</b>\n\n"
            f"<b>Запрос:</b> {escape(scene)}\n"
            f"<b>Распознана общая поза:</b> <code>{escape(requested_posture)}</code>\n"
            f"<b>Выбрана Depth:</b> <code>{escape(relative_depth)}</code>\n"
            f"<b>pose_style:</b> <code>{escape(str(pose_metadata.get('pose_style', 'unknown')))}</code>",
            parse_mode="HTML",
        )
        await message.answer(
