...
async def _run_pose_folder_test(message: types.Message, ctx: TelegramContext, folder_name: str) -> None:
    ...
    for index, (bone_path, depth_path, stem) in enumerate(pairs, start=1):
        ...
        try:
            ...
            if face: result = await image_service.image_provider.reface(image=result, face_reference=face)
            await message.answer_photo(types.BufferedInputFile(depth_path.read_bytes(), filename=depth_path.name), caption=f"🗺 Depth: <code>{stem}</code>", parse_mode="HTML")
            ...
            await message.answer_photo(types.BufferedInputFile(result, filename=f"{stem}_result.png"), caption=f"🧪 Результат: <code>{stem}</code>", parse_mode="HTML", reply_markup=keyboard)
